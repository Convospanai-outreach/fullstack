    ...
    try {
        ...
        switch (action) {
            ...
            case "assign-cluster": {
                ...
                const leadRecord = await prisma.lead.findUnique({ where: { id: leadId }, select: { teamId: true } });
                if (!leadRecord || leadRecord.teamId !== ctx.teamId) {
                    return NextResponse.json({ error: "Lead not found" }, { status: 404 });
                }
                ...
                const features = {
                    value: Math.min((leadDetails.value || 0) / 10000, 1),
                    engagement: leadDetails.intentScore || 0,
                    recency: Math.max(1 - ((Date.now() - leadDetails.updatedAt.getTime()) / (90 * 24 * 60 * 60 * 1000)), 0),
                    frequency: Math.min((leadDetails.emails.length + leadDetails.messages.length) / 20, 1)
                };
    ...
    try {
        ...
        return NextResponse.json({
            ...
            data: {
                clusters,
                summary: {
                    totalLeads: clusters.reduce((sum: number, c: { memberCount: number }) => sum + c.memberCount, 0),
                    totalValue: clusters.reduce((sum: number, c: { averageValue: number; memberCount: number }) => sum + (c.averageValue * c.memberCount), 0)
                }
            }
        });
    } catch (error: any) {
