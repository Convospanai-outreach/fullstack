...
function postRequest(body: unknown) {
    return new NextRequest("http://localhost:3001/api/learning/enrich-lead", {
        method: "POST",
        body: JSON.stringify(body),
    });
}

describe("POST /api/learning/enrich-lead", () => {
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    it("enqueues enrichment for a lead in the caller's own team", async () => {
        ...
        const response = await POST(
            postRequest({ leadId: "lead-1", campaignId: "campaign-1", extraction: { foo: "bar" } })
        );
