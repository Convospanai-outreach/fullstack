...
// (see wabaCredentials.ts, smtpConfigService.ts) - duplicated per this repo's
// established precedent of duplication over cross-module coupling.
// SsoConfiguration.clientSecret is a plain String column (not Json), so the
// encrypted payload is JSON-stringified into it.
...
// Rows written before this fix still hold a plaintext secret - falls back to
// returning the raw string so those configs keep working until their next save.
