...
| `settings/branding/page.tsx`'s logo field: "Upload feature coming soon. Use a public URL for now." | **Fixed (user-approved: Vercel Blob).** Provisioned a real Blob store (`craftmyfunnel-branding`, public access) via `vercel blob create-store`, linked to the `fullstack-web-xkxn` project with `BLOB_READ_WRITE_TOKEN` set in all three environments. Added `POST /api/settings/branding/logo` (validates PNG/JPEG/WebP/SVG, 2MB max, uploads via `@vercel/blob`'s `put()`, returns the public URL) and wired the branding page's Logo field to a real file picker with a live preview, falling back to the existing raw-URL text input for anyone who wants to paste one directly. **Not independently re-verified live** — same blocker as the step editor above (local Clerk is keyless/invite-gated, and `put()` needs the Blob token that's only present in Vercel's env, not a local shell), so this is code-complete and traced but not click-tested in a browser this session |
| **Discovered while wiring the logo upload — `POST /api/settings/branding` was a permanent no-op stub.** The entire "Save Changes" button on the branding page always returned `{ gated: true }` regardless of input; nothing was ever persisted, for any field, ever. This is a distinct, arguably bigger bug than the missing upload — the whole settings form was decorative. Meanwhile `Team.branding` (a `Json?` column shaped exactly `{ logoUrl, primaryColor, portalTitle }`) was already being written for real by `/setup`'s step-2 save handler and read by `brandingService.ts` (custom-domain white-labeling) and `AgentExecutor.ts` (agent context) | **Fixed** — `POST /api/settings/branding` now merges into and persists `Team.branding` the same way `/setup/save` step 2 does; added a `GET` handler so the page hydrates existing values on load instead of always starting blank |
| **Discovered while fixing the above — the entire `/settings/*` section (19 pages) has zero app chrome in production.** Same bug class as the `DASHBOARD_PREFIXES` sweep two rounds ago: `LayoutShell.tsx` lists `/settings` as a dashboard route, but `src/app/settings/*` was never inside the `(dashboard)` route group — it only has its own small local nav (`src/app/settings/layout.tsx`, a 15-item settings-section sidebar), so every settings page was missing the app's main sidebar/header entirely, with no way back into the rest of the app except editing the URL. This is a much bigger blast radius than the earlier 13-page batch — it's the entire settings surface | **Fixed** — moved all 19 pages (`agent`, `approvals`, `audit`, `branding`, `budgeting`, `crm`, `features`, `general`, `governance`(`/analytics`), `guardrails`, `hitl`, `keys`, `mailboxes`, `notifications`, `sso`, `team`, `webhooks`, plus the section's own `layout.tsx` and index `page.tsx`) into `(dashboard)/settings/`, same pattern as the earlier batch — absolute `@/` imports throughout, so no import changes were needed |
| **Also found during the same `LayoutShell` pass — the opposite bug, duplicate chrome.** `(marketing)/layout.tsx` renders its own `NavBar`+`Footer`, but `accept-invite`, `agent-login`, and `client-login` live inside that route group without being listed in `MARKETING_PREFIXES`, so `LayoutShell` also rendered the generic `Header`+`Footer` on top — double nav/footer on all three pages | **Fixed** — added the three paths to `MARKETING_PREFIXES` |
...
| `DASHBOARD_PREFIXES` includes `/command-center`, which has no corresponding page anywhere in the route tree (top-level or in `(dashboard)`) | **Flagged only, not fixed** — dead array entry, doesn't affect any real page, so no user-facing bug; not removing per repo convention against deleting unrelated dead code unasked |
