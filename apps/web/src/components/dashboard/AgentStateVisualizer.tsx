...
const STAGES: { id: AgentState; label: string; icon: any; color: string }[] = [
    { id: 'PENDING', label: 'Idle', icon: Circle, color: 'text-slate-500' },
...
export const AgentStateVisualizer: React.FC<AgentStateVisualizerProps> = ({ currentState, taskId }) => {
    ...
    return (
        <div className="p-6 bg-slate-900/50 border border-slate-700/50 rounded-2xl glass-morphism overflow-hidden relative">
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-slate-400 text-xs mt-1 font-mono">TASK_ID: {taskId || '0x000000'}</p>
                </div>
                        ...
                        <div key={stage.id} className="flex flex-col items-center gap-3 group relative">
                            ...
                            <div className={`
                                ...
                                ${isCompleted ? 'bg-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.5)]' :
                                    isActive ? 'bg-slate-800 border-2 border-indigo-500 scale-110 shadow-[0_0_20px_rgba(99,102,241,0.3)]' :
                            ...
                            `}>
                                ...
                                {/* Hover Tooltip */}
                                <div className="absolute -top-8 bg-slate-950 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 border border-slate-700">
                                    {stage.label}
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col items-center text-center">
                                <span className={`text-[10px] font-bold tracking-tighter transition-colors ${isActive ? 'text-white' : 'text-slate-500'}`}>
                                    {stage.id}
                                </span>
                                {isActive && (
