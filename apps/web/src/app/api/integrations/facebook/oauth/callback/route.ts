...
export async function GET(req: NextRequest) {
    ...
    try {
        const result = await connectFacebookPages({ code, state });
        const targetPath = result.nextPath || "/settings/crm";
        const redirectUrl = new URL(targetPath, baseUrl);
        redirectUrl.searchParams.set("connected", "true");
        redirectUrl.searchParams.set("pages", String(result.pages.length));
