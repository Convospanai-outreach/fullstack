...
describe("POST /orchestrator/agents/[id]/run - double-run guard", () => {
    ...
    it("passes teamId in both the job payload and options (OPEN-202: omitting it let the worker fall back to an unscoped cross-tenant query and skip credit billing)", async () => {
        mockPrisma.agent.updateMany.mockResolvedValue({ count: 1 });
