...
import { ArrowRight, CheckCircle, ChevronDown, ClipboardCheck, Cpu, Database, HardDrive, Network, PhoneCall, ShieldCheck, Sparkles, TrendingUp } from "lucide-react";
...
const architectureLayers = [
  { title: "NetJana", tag: "Buyer Signals", solves: "Who should I contact?", points: ["Intent detection engine", "Corporate activity tracking", "Decision-maker tracking", "Trigger event discovery"] },
  { title: "CMF Core", tag: "AI Outreach SaaS", solves: "How do I reach them?", points: ["Automated email sequences", "LinkedIn outbound rails", "WhatsApp business workflows", "Governance & compliance"] },
  { title: "Human Layer", tag: "Conversion Moat", solves: "How do I secure meetings?", points: ["Managed human callers", "B2B appointment fixing", "Immediate verification", "High-trust pipeline nurturing"] },
  { title: "Covospan EDGE", tag: "Sovereign Edge Node", solves: "How do I deploy safely?", points: ["Raspberry Pi 5 edge device", "Secure boot & firmware control", "Encrypted local storage", "On-device logs & tokenisation"] },
];
const edgeTiers = [
...
export default function Home() {
  return (
    <div className="min-h-screen overflow-hidden bg-[#050d17] text-slate-100">
      ...
      <section className="relative overflow-hidden border-y border-white/8 bg-[#02070d] py-24"><div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(16,185,129,0.18),transparent_30%),radial-gradient(circle_at_80%_60%,rgba(34,211,238,0.14),transparent_28%)]" /><div className="relative mx-auto max-w-7xl px-6"><div className="max-w-3xl"><div className="mb-5 inline-flex rounded-full border border-emerald-300/20 bg-emerald-300/10 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-emerald-200"><Cpu className="mr-2 h-4 w-4" />Covospan EDGE</div><h2 className="text-4xl font-black text-white sm:text-5xl">Sovereign edge layer for safe deployment</h2><p className="mt-4 text-lg text-slate-400">Covospan EDGE uses a Raspberry Pi 5 based edge node as the optional on-prem layer for secure boot, encrypted storage, local memory logs and governed AI processing close to the data.</p></div><div className="mt-12 grid gap-5 lg:grid-cols-4">{architectureLayers.map((layer, index) => <GlassCard key={layer.title} className="p-6"><div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl border border-emerald-300/20 bg-emerald-300/10 text-emerald-200">{index === 0 ? <Network className="h-5 w-5" /> : index === 1 ? <Sparkles className="h-5 w-5" /> : index === 2 ? <PhoneCall className="h-5 w-5" /> : <HardDrive className="h-5 w-5" />}</div><h3 className="text-2xl font-black text-white">{layer.title}</h3><p className="mt-2 inline-flex rounded-full bg-emerald-300/10 px-3 py-1 text-xs font-black uppercase tracking-[0.16em] text-emerald-200">{layer.tag}</p><p className="mt-5 text-sm font-bold text-slate-200">Solves: {layer.solves}</p><ul className="mt-4 space-y-3 text-sm text-slate-400">{layer.points.map((point) => <li key={point} className="flex gap-2"><CheckCircle className="mt-0.5 h-4 w-4 shrink-0 text-emerald-300" />{point}</li>)}</ul></GlassCard>)}</div><div className="mt-10 grid gap-4 lg:grid-cols-3">{edgeTiers.map((tier) => <GlassCard key={tier.title} className="p-5"><Database className="mb-4 h-5 w-5 text-cyan-300" /><h3 className="font-black text-white">{tier.title}</h3><p className="mt-2 text-sm leading-6 text-slate-400">{tier.text}</p></GlassCard>)}</div></div></section>
