        ...
        <div className="space-y-8 max-w-5xl mr-auto">
            ...
            {newKey && (
                <div className="p-6 bg-green-900/20 border border-green-500/50 rounded-xl space-y-4">
                    ...
                    <div className="flex gap-2">
                        <code className="flex-1 bg-background p-3 rounded font-mono text-foreground overflow-x-auto">
                            {newKey}
                        </code>
                        <button onClick={copyKey} className="p-3 bg-muted hover:bg-accent rounded text-foreground">
                            <Copy className="w-5 h-5" />
                        </button>
                    </div>
                    <button onClick={() => setNewKey(null)} className="text-sm text-muted-foreground hover:text-foreground underline">
                        I have saved it
                    </button>
                </div>
            )}
            ...
            <GlassCard className="p-6">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-bold text-foreground">Active Keys</h3>
                    <button
                ...
                {showCreate && (
                    <div className="mb-6 p-4 bg-muted rounded-lg flex items-end gap-3 animated fadeIn">
                        <div className="flex-1 space-y-2">
                            <label className="text-xs text-muted-foreground uppercase">Key Name</label>
                            <input
                                autoFocus
                                type="text"
                        ...
                        <button onClick={handleCreate} className="px-4 py-2 bg-green-600 text-foreground rounded font-medium">
                            Generate
                        </button>
                    </div>
                )}
                ...
                <div className="space-y-4">
                    {keys.map(key => (
                        <div key={key.id} className="flex justify-between items-center p-4 bg-muted rounded-lg border border-border">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2 mt-2">
                                        {key.scopes.map((scope: string) => (
                                            <span key={scope} className="text-[10px] px-2 py-0.5 bg-muted rounded-full text-foreground">
                                                {scope}
                                            </span>
                                        ))}
                                    </div>
                            ...
                            <button
                                ...
                                className="text-destructive hover:text-red-300 hover:bg-red-400/10 p-2 rounded"
                            >
                    ...
                    {keys.length === 0 && !loading && (
                        <div className="text-center py-8 text-muted-foreground italic">No active API keys.</div>
                    )}
                </div>
            </GlassCard>
        </div>
