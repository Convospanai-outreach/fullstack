...
|----|------|----------|--------|--------------------------------|
| **OPEN-01** | Multi-Tenant Data Isolation | Security | **Resolved** | Verified write (`updateMany`), read (`findFirst`), and delete (`deleteMany`) paths enforce `teamId` scoping. Live test `scripts/test-tenant-isolation.mjs` confirmed Team A update/read/delete on Team B lead returns 0 rows modified. |
| **OPEN-02** | Remote GitHub Actions CI Execution | CI/CD | **Still Open** | Awaiting actual GitHub Actions job log/URL for pushed commit `0018280`. |
...
| **OPEN-09** | Gmail PubSub / History Inbound Reply Processing Engine | Architecture Gap | **Still Open** | `pubsub/route.ts` saves `historyId` and `imap-reply-sync-worker.ts` handles SMTP, but no worker invokes `users.history.list` to parse incoming `In-Reply-To` / `References` headers, emit `REPLY_RECEIVED` `emailEvent`, and advance lead status to `REPLIED`. |
