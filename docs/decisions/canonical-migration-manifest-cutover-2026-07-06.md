...
| 5 | Canonical `packages/db` migration adoption implementation PR | Create the canonical `packages/db` migration history from the approved manifest and classifications | May copy or adopt migration files after approvals; may update docs/tooling tied to canonical ownership | No production DB execution | Canonical `packages/db` history exists in reviewed form |
