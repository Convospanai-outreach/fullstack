...
| Live public URL recheck after public route fix | `/security`, `/support`, `/data-deletion`, and `/google-api-disclosure` should be public after commit `74423bc` | `docs/audits/live-url-approval-readiness-output.md` recheck still shows all four routes returning initial `307` redirects to `/login?callbackUrl=...` | FAIL | approval-readiness-agent | Live custom domain is not reflecting expected public access |
| Support email live recheck after email fix | `/terms` and `/contact` should show `support@craftmyfunnel.live` with no old emails after commit `74423bc` | `docs/audits/live-url-approval-readiness-output.md` recheck still finds `bizcomm.soulutions@gmail.com` on `/terms` and `support@craftmyfunnel.com` + `enterprise@craftmyfunnel.com` on `/contact` | FAIL | approval-readiness-agent | Live custom domain is not reflecting expected email content |
| Public route allowlist fix | `/security`, `/support`, `/data-deletion`, and `/google-api-disclosure` should be unauthenticated public routes | `apps/web/src/proxy.ts` adds all four paths to `publicPaths` | PASS | approval-readiness-agent | Requires Vercel deployment and live recheck |
| Support email source fix | Public source pages should use `support@craftmyfunnel.live` | `apps/web/src/app/terms/page.tsx` and `apps/web/src/app/contact/page.tsx` now use `support@craftmyfunnel.live` | PASS | approval-readiness-agent | Requires Vercel deployment and live recheck |
