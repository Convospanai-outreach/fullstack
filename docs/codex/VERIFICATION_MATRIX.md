...
| Public page auth-session client errors | Public pages should not emit avoidable auth/session client errors | Chromium smoke observed `/api/auth/session` returning `500` or `429` and NextAuth client errors on `/funnel` and approval pages | RUNTIME_RISK | approval-readiness-agent | Pages still render, but this blocks overall readiness upgrade |
| Latest-head local web validation | Lint, typecheck, and build should pass locally or have equivalent evidence | `npm run typecheck --workspace apps/web` passed; `npm run lint --workspace apps/web` timed out after 180s; `npm run build --workspace apps/web` timed out after 600s | RUNTIME_RISK | approval-readiness-agent | Vercel status is green, but local validation is insufficient |
