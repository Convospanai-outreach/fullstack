...
## Migration prerequisite decision (2026-07-08)

| Check | Expected | Actual safe evidence | Verdict | Owner agent | Notes |
| --- | --- | --- | --- | --- | --- |
| PR #93 Greptile clean state carried forward | The prerequisite decision should start from the latest governance baseline, not reopen the already-fixed blocked-proof matrix issue | The new decision record and workflow state carry forward the PR #93 baseline where the blocked-proof matrix row is already corrected and the live DB proof still remains `BLOCKED` | PASS | migration-safety-agent | This PR records the next prerequisite decision only. |
| live DB proof remains BLOCKED | The decision must not reclassify the failed live proof as PASS | The decision record and workflow state both keep live DB proof status at `BLOCKED` | PASS | migration-safety-agent | BLOCKED state remains explicit and owned. |
| missing public app schema carried forward | The decision should preserve the proved missing-schema state from the read-only evidence | The decision record carries forward that the checked Supabase DB has no public app schema | PASS | migration-safety-agent | This is evidence, not readiness. |
| `_prisma_migrations` missing evidence carried forward | The decision should preserve the missing migration-metadata state | The decision record and prior evidence both keep `public._prisma_migrations` missing | PASS | migration-safety-agent | No migration metadata is available in the checked live DB. |
| migration execution remains NOT_APPROVED | The decision must not authorize migration execution | Decision record and workflow state both keep migration execution at `NOT_APPROVED` | PASS | migration-safety-agent | No migration execution is authorized by this PR. |
| staging dry-run remains NOT_RUN / PENDING | The decision must not imply staging execution has started | Decision record and workflow state both keep staging dry-run at `NOT_RUN / PENDING` | PASS | migration-safety-agent | Staging execution still requires reviewer sign-off. |
| production migration remains NOT_APPROVED | The decision must keep production migration blocked | Decision record and workflow state both keep production migration at `NOT_APPROVED` | PASS | migration-safety-agent | Production work remains outside this docs-only PR. |
| canonical owner remains packages/db/prisma | The prerequisite path should preserve the accepted target owner | Decision record keeps `packages/db/prisma` as the target canonical owner | PASS | migration-safety-agent | Matches prior ownership and cutover decisions. |
| migration histories require manifest-backed reconciliation | The prerequisite path must preserve manifest-backed adoption rather than blind copy | Decision record requires `apps/web/prisma` and `apps/api/prisma` histories to be reconciled through manifest-backed adoption with explicit classification for non-shared items | PASS | migration-safety-agent | No blind copy or baseline-only shortcut is approved here. |
| EdgeNode DELETE remains RED / unapproved | The prerequisite decision must preserve the destructive EdgeNode blocker | Decision record keeps historical `DELETE FROM "EdgeNode"` RED and unapproved | PASS | migration-safety-agent | No destructive approval language was added. |
| destructive scanner remains advisory while EdgeNode RED | Scanner language must not imply clearance while the known destructive path remains unresolved | Decision record and prior runbook context both preserve that scanner results are reviewed as advisory while EdgeNode remains RED | PASS | migration-safety-agent | No scanner allowlist is approved. |
| PR #6 remains BLOCKED | Mailbox migration work must remain blocked until compatibility is proven | Decision record and workflow state both keep PR #6 at `BLOCKED` | PASS | migration-safety-agent | ConnectedMailbox proof is still required. |
| production readiness remains NOT_READY | The prerequisite decision must not imply readiness | Decision record and workflow state both keep production readiness at `NOT_READY` | PASS | migration-safety-agent | No readiness claim is approved. |
| DB/migration governance remains RED / NEEDS_REPLAN | The decision must preserve the unresolved governance state | Decision record and workflow state both keep DB/migration governance at `RED / NEEDS_REPLAN` | PASS | migration-safety-agent | Governance remains unresolved. |
| no DB access occurred in this PR | Docs-only prerequisite work must not touch a database | No DB access occurred in this PR | PASS | migration-safety-agent | Planning only. |
| no SQL executed in this PR | Docs-only prerequisite work must not run SQL | No SQL was executed in this PR | PASS | migration-safety-agent | No live queries were run. |
| no seed ran in this PR | The prerequisite decision must not trigger seed behavior | No seed execution occurred in this PR | PASS | migration-safety-agent | Seed paths remain out of scope. |
| no migration ran in this PR | The prerequisite decision must not run migration tooling | No migration execution occurred in this PR | PASS | migration-safety-agent | Migration execution remains `NOT_APPROVED`. |
| no secrets/env values accessed in this PR | The prerequisite decision must not access secret material | No secrets or env values were accessed in this PR | PASS | migration-safety-agent | Credentials remain outside Codex and GitHub. |
