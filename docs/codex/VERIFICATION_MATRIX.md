...
| API_INTERNAL_ORIGIN | present if web calls API internally | Public custom-domain checks of `/api/proxy` and `/api/proxy/health` return unauthenticated `401` before proxying; repo source falls back to `API_BASE_URL` or `http://localhost:3001`; Railway statuses do not expose canonical API origin | BLOCKED_EXTERNAL_ACCESS | post-pr39-production-smoke-agent | Exact active API origin/custom domain must be confirmed in Railway/Vercel dashboards; do not guess |
...
| Health ready | DB/schema/migration/env marker | Latest custom-domain `/api/health` on `www.craftmyfunnel.live` returned `503 Service Unavailable` with `checks.database: "down"` | FAIL | post-pr39-production-smoke-agent | Requires dashboard/env/DB verification; no production data changes were made |
...
| PR #35 high security audit gate | `npm audit --audit-level=high --omit=dev` should pass before CI proceeds to Prisma/typecheck/lint/test/build | CI logs showed high `nodemailer` and `ws` findings; targeted fix updated Nodemailer to `9.0.1` and resolved `ws` to `8.21.0`; local high audit exits 0, and GitHub rerun on `b08bf9579a7ee5122f8f806ca3387f79ff5666e6` confirmed Security Audit passed in both web workflows | PASS | npm-lockfile-ci-stability-agent | Low/moderate findings remain for Stage 13 |
...
| Post-PR35 root high audit | `npm audit --audit-level=high --omit=dev` should pass on latest main | Command passed with exit code 0; remaining output lists 1 low and 6 moderate findings only | PASS | post-pr35-release-gate-agent | Low/moderate findings remain Stage 13 work |
...
| Production high audit gate | `npm audit --audit-level=high --omit=dev` passes | Post-PR35 merge recheck on latest main `e14806c` passed with exit code 0; only low/moderate findings remain in npm audit output | PASS | dependency-security-agent | GitHub Dependabot alert mapping still remains separate Stage 13 work |
