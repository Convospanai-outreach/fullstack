...
| Health ready | DB/schema/migration/env marker | Latest custom-domain `/api/health` and `/api/health?probe=ready` returned `503 Service Unavailable` with `checks.database: "down"` | FAIL | api-origin-health-readiness-agent | Readiness uses `DATABASE_URL` and Prisma `SELECT 1`; requires Vercel env/DB connectivity confirmation without production data mutation |
