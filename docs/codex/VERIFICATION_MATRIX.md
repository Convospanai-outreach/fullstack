...
| Public page auth-session client errors | Public pages should not emit avoidable auth/session client errors | Post-env-redeploy checks show `/api/auth/session` calls successfully returning `200 OK` (empty object); client-side configuration errors are resolved | PASS | approval-readiness-agent | Public pages still request session because custom production domain is serving branch `main` |
...
| GitHub Actions branch runs | Current commit should have Actions runs for lint/typecheck/build/tests | GitHub check-runs API for head `ef4eaf2` only returned Vercel Preview Comments success and Supabase Preview skipped; no Actions runs found | MISSING | ci-gate-agent | See `docs/audits/github-actions-status-check.md` |
