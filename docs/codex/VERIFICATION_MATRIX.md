...
| Live public URL status | Required approval URLs must be public without login before submission | `docs/audits/live-url-approval-readiness-output.md` shows `/`, `/privacy`, `/terms`, `/contact`, `/help`, `/faq` public `200`; `/security`, `/support`, `/data-deletion`, `/google-api-disclosure` redirect to login | FAIL | approval-readiness-agent | Approval readiness needs replan before submission |
| Approval support email domain | Public approval/support pages should consistently use `support@craftmyfunnel.live` | `/terms` uses `bizcomm.soulutions@gmail.com`; `/contact` uses `support@craftmyfunnel.com` and `enterprise@craftmyfunnel.com` | FAIL | approval-readiness-agent | Align before Google/Chrome submission |
