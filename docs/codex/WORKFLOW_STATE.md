...
| Next action | Confirm GHCR web Docker image workflow from a visible workflow_dispatch run or post-merge main push, then resolve Railway duplicate/stale service statuses, API_INTERNAL_ORIGIN, DB/schema drift, and Stage 13 dependency alert mapping |
...
| Main release gate not fully green | ci-gate-agent | Latest main `e14806c` has requested workflows green (`CI`, `Production Readiness Gate`, `Vercel Parity Build`, `Phi-3 Verification`) and Vercel green, but `Register Docker Images to GHCR` fails at web Docker `next build` because `nodemailer` cannot be resolved; Railway statuses include two failures and one pending service. PR #37 head `bc52b95` applies a Dockerfile hotfix that copies `apps/web/node_modules` from the deps stage into the builder stage; local non-Docker validations and normal PR checks pass, but no visible post-fix GHCR dispatch run was available during recheck. | Confirm GHCR from visible dispatch/run URL or post-merge main push, then resolve Railway duplicate/stale service statuses | BLOCKED_BY_FAILED_TESTS |
...
| Web Docker GHCR nodemailer hotfix | npm-lockfile-ci-stability-agent | NEEDS_REPLAN | docs/audits/web-docker-nodemailer-build-fix.md | Root cause: npm installs `nodemailer` under `apps/web/node_modules`, but `apps/web/Dockerfile` builder stage copied only root `node_modules`. Fix copies `/repo/apps/web/node_modules` from deps to builder. Local high audit, npm 10 dry-run, web typecheck, and web build pass; PR #37 normal checks are green. Local Docker unavailable and no visible post-fix GHCR dispatch run was found, so GHCR confirmation is still required. |
