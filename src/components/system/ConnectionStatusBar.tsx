...
export function ConnectionStatusBar() {
    ...
    const fetchStatus = useCallback(async () => {
        ...
        try {
            ...
            // silently fail — bar will just show loading
        } finally {
            setIsLoading(false);
        }
    }, []);
    ...
    if (isLoading && !systemStatus) {
        return (
            <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2 bg-slate-800/80 backdrop-blur border border-white/10 rounded-full px-3 py-1.5 shadow-xl text-xs text-slate-400">
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50">
            ...
            <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-full shadow-xl border transition-all duration-200 text-xs font-medium backdrop-blur
                    ${overallColors.bg} ${overallColors.ring} ring-1 border-white/10 hover:ring-2 hover:scale-105`}
            >
